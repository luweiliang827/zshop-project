...
<head>
    ...
    <script>
        ...
        function addProductType() {
            $.post(
                '${pageContext.request.contextPath}/backend/productType/addProductType',
                {
        ...
        <%--function updateProductType(){--%>
            <%--var id = $('#proTypeNum').val();--%>
            <%--var name = $('#proTypeName').val();--%>
            <%--$.post(--%>
                <%--'${pageContext.request.contextPath}/backend/productType/updateProductType',--%>
                <%--{id:id,name:name},--%>
                <%--function (result) {--%>
                    <%--if(result.status == 1){--%>
                        <%--layer.msg(result.message,{time:2000,skin:'successMsg'});--%>
                    <%--}else{--%>
                        <%--layer.msg(result.message,{time:2000,skin:'errorMsg'});--%>
                    <%--}--%>
                <%--}--%>
            <%--);--%>
        <%--}--%>
        ...
        function updateProductType(){
            $.ajax({
                ...
                success:function (result) {
                    if(result.status == 1){
                        layer.msg(result.message,
                            {time:500,skin:'successMsg'},
                            function () {
                            //重新加载数据
                            location.href='${pageContext.request.contextPath}/backend/productType/findAll?pageNum='+${pageInfo.pageNum};
                        });
                    }else{
                        layer.msg(result.message,{time:1000,skin:'errorMsg'});
                    }
                }
            });
        }
        ...
        function showDelModel(id) {
            ...
            $('#delProductTypeModal').modal('show');
        }
        ...
        function updateStatus(id,btn){
            $.post(
                ...
                function (result) {
                    if(result.status == 1){
                        //局部刷新,parent()该标签的上一级，prev()上一级的前门一个标签
                        let $td = $(btn).parent().prev();
                        //状态那一列为”启用“
                        if($td.text().trim() == '启用'){
                            //将状态那一列修改为”禁用“
                            $td.text('禁用');
                            //将按钮修改为“启用”并删除之前按钮的样式在添加对应的样式
...
<body>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <c:if test="${productType.status == 1}">
                                        <input type="button" class="btn btn-danger btn-sm doProDisable" value="禁用" onclick="updateStatus(${productType.id},this)">
                                    </c:if>
                                    <c:if test="${productType.status == 0}">
                                        <input type="button" class="btn btn-success btn-sm doProDisable" value="启用" onclick="updateStatus(${productType.id},this)">
                                    </c:if>
                                </td>
                            </tr>
                        </c:forEach>
    ...
    <!-- 确认删除商品类型 start -->
    <div class="modal fade" tabindex="-1" id="delProductTypeModal">
        <!-- 窗口声明 -->
